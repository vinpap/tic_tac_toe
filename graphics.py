...
class Graphics(Graphics_interface):
    ...
    def wait_for_move(self, current_board):
        ...
        self.screen.blit(
            self.title_surface,
            (
                self.window_size[0] * 0.5 - (self.title_surface.get_width() / 2),
    ...
    def update_display(self, current_board):
        ...
        while running:
            ...
            if time.time() - starting_time >= time_per_ai_move:
                running = False
    ...
    def draw_grid(self, current_board):
        ...
        space_width = round(
            (self.window_size[0] * 0.85 - self.window_size[0] * 0.15)
            / self.current_board.shape[1]
        )
        space_height = round(
            (self.window_size[1] * 0.85 - self.window_size[1] * 0.15)
            / self.current_board.shape[0]
        )
        x_index = 0
        y_index = 0
        for x in range(
            round(self.window_size[0] * 0.15),
            round(self.window_size[0] * 0.85),
            space_width,
        ):
            for y in range(
                round(self.window_size[1] * 0.15),
                round(self.window_size[1] * 0.85),
                space_height,
            ):
        ...
        for index, value in np.ndenumerate(current_board):
            if value == 1:
                ...
                pygame.draw.circle(
                    self.screen, (0, 0, 255), pos, 0.45 * space_height, 5
                )
            elif value == 2:
                x = round(self.window_size[0] * 0.15 + space_width * index[1])
                y = round(self.window_size[1] * 0.15 + space_height * index[0])
    ...
    def display_scores(self):
        ...
        for l in p2_lines:
            ...
            self.screen.blit(
                surface,
                (
                    self.window_size[0] * 0.85,
                    self.window_size[1] * starting_height_ratio,
                ),
            )
            starting_height_ratio += 0.05
    ...
    def process_click(self, click_position_x, click_position_y):
        ...
        space_height = round(
            (self.window_size[1] * 0.86 - self.window_size[1] * 0.15)
            / self.current_board.shape[0]
        )
        for s in self.grid:
